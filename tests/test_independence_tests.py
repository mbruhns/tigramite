...
def gen_data_sample(seed, corr_val, T):
    ...
    array = np.random.multivariate_normal(
        mean=np.zeros(dim), cov=cov, size=T
    ).T
...
def check_get_array(ind_test, sample):
    ...
    a_array, a_xyz, a_xyz_nodes = ind_test._get_array(
        x_nds, y_nds, z_nds, tau_max
    )
    # Get the array directly from the dataframe
    b_array, b_xyz, b_xyz_nodes = dataframe.construct_array(
        x_nds,
        y_nds,
        z_nds,
        tau_max=tau_max,
...
def test_parcorr(par_corr, data_sample_a):
    ...
    np.testing.assert_allclose(
        np.array(corr_val), np.array(val_est), atol=0.02
    )
...
def test_gpdc_residuals(gpdc, seed):
    ...
    np.testing.assert_allclose(
        pred[cntr], func(target_res[cntr], c_val), atol=0.2
    )
