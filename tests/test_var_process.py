...
@pytest.fixture(
    params=[
        ...
        (100.0, 0.99, 1000, 1, 1, "Default Exp Decay"),
        (-100.0, 0.99, 1000, 1, 1, "Negative Exp Decay"),
        (100.0, 0.99, 10, 1, 1, "Short Exp Decay"),
        (100.0, 0.99, 1000, 10, 1, "Delayed Exp Decay"),
        (100.0, 0.99, 1000, 1, 2, "2D Exp Decay"),
    ]
)
def decoupled_exp_decay_process(request):
    ...
    init_vals = np.array(
        [gen_decay(init, decay, delay + 1, delay) for _ in range(dim)]
    )
    ...
    expect = np.vstack(
        [gen_decay(init, decay, n_times, delay) for _ in range(dim)]
    ).T
...
def gen_process(a_process):
    ...
    data, true_parents_neighbors = pp.var_process(
        coefs, T=max_time, initial_values=init_vals, use="no_noise"
    )
...
def check_process_data(name, coefs, data, expect):
    ...
    np.testing.assert_allclose(
        data, expect, rtol=1e-10, verbose=False, err_msg=error_message
    )
...
def test_bad_parameters(bad_parameter_sets):
    ...
    error_message = (
        message
        + " should trigger a value error for var_process "
...
def unstable_parameter_sets(request):
    ...
    for delay in range(max_delay):
        for a_params, a_coef in zip(
            [stab_params, unst_params], [stab_coef, unst_coef]
        ):
            node_id = delay
    ...
    return (
        pp._get_lag_connect_matrix(stab_params),
...
def test_stability_parameters(unstable_parameter_sets):
    ...
    error_message = (
        message
        + " should trigger an assertion error for "
        + "_var_network graph."
    )
    # Test the good parameter set
    try:
...
def covariance_parameters(request):
    ...
    good_params = {}
    good_params[0] = [
        ((1, 0), default_coef * 1.0),
        ((2, 0), default_coef * 3.0),
    ]
    good_params[1] = [
        ((2, 0), default_coef * 2.0),
        ((0, 0), default_coef * 1.0),
    ]
    good_params[2] = [
        ((0, 0), default_coef * 3.0),
        ((1, 0), default_coef * 2.0),
    ]
    good_params[3] = [((3, 0), default_coef * 4.0)]
...
def test_covariance_construction(covariance_parameters):
    ...
    for j, i, _, coeff in pp._iter_coeffs(good_params):
        ...
        err_message = (
            "Node {} and parent node {} have".format(j, i)
...
def test_noise_generation(covariance_parameters):
    ...
    data, _ = pp.var_process(
        good_params, T=10000, use="inno_cov", verbosity=0, initial_values=None
    )
    # Get the covariance of the data set
    covar_result = np.cov(data.T)
    err_message = (
        "Covariance of data does not match covariance implied by "
        + " parameter set"
    )
    np.testing.assert_allclose(
        covar_matrix,
